...
初始化如果需要使用自定义的配置文件，请查看[配置文件](/zh/guide/config)一章。注意初始化方式分不同的版本。
...
当然如果你想实现更加复杂的操作，比如增加自有插件配置、设置插件名字或者想把你的插件开放给更多人使用，欢迎查阅[插件开发](/zh/dev-guide/cli)相关章节。
...
详细参考 [api-pluginHandler](/zh/api/#pluginhandler)。
...
详细参考 [api-pluginLoader](/zh/api/#pluginloader)。
